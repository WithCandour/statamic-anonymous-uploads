...
class FormController extends Controller
{
    ...
    protected function normalizeAssetsValues($fields, $request)
    {
        ...
        return $fields->all()
            ->filter(function ($field) {
                return ($field->fieldtype()->handle() === 'assets' || $field->fieldtype()->handle() === 'anonymous_uploads') && request()->hasFile($field->handle());
            })
    ...
    protected function extraRules($fields)
    {
        $assetFieldRules = $fields->all()
            ->filter(function ($field) {
                return $field->fieldtype()->handle() === 'assets' || $field->fieldtype()->handle() === 'anonymous_uploads';
            })
